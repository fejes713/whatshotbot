...
function ContentIdeaExplorer() {
    ...
    return [
      {
        ...
        duration: "2:00",
        description:
          "Set up the context and hook the viewer with an engaging opening.",
      },
      {
        title: "Main Concept",
        duration: "5:30",
        description:
          "Deep dive into the core topic with detailed explanations.",
      },
      {
        title: "Examples",
        duration: "4:15",
        description:
          "Demonstrate practical applications and real-world scenarios.",
      },
      {
        title: "Advanced Tips",
        duration: "3:45",
  ...
  const exploreIdeasRef =
    useRef<(channelData: any, sourceNodeId: string) => void>();
  ...
  exploreIdeasRef.current = useCallback(
    async (channelData: any, sourceNodeId: string) => {
      const sourceNode = nodes.find((node) => node.id === sourceNodeId);
      if (!sourceNode) return;
      ...
      try {
        ...
        const baseX = sourceNode.position.x + HORIZONTAL_SPACING;
        const baseY =
          sourceNode.position.y - ((ideas.length - 1) * VERTICAL_SPACING) / 2;
        ...
        const newNodes: Node[] = ideas.map((idea, index) => ({
          ...
          data: {
            ...
            onCreate: () =>
              createVideoStructureRef.current?.(
                idea,
                `${sourceNodeId}-idea-${index}`
              ),
          },
